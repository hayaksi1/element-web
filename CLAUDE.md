...
4.  **Execute:** Apply changes systematically after the plan is established. Format with oxfmt (`pnpm lint:fmt-fix`) and ensure strict TypeScript rules are followed.
...
- **Language & Formatting:**
    ...
    - FORMATTING: Every file must be formatted with oxfmt (`pnpm lint:fmt-fix`).
- **Architecture:**
...
- **VERIFY:** Use `grep_search`, `view_file`, or `list_dir` to inspect the codebase _before_ making any changes.
...
- Before committing or completing a task, run linting and unit tests:
    ...
    - Auto-fix formatting: `pnpm lint:fmt-fix` (oxfmt; `pnpm lint:fmt` = `oxfmt --check`). Upstream removed Prettier (#33844); the old `pnpm lint:prettier-fix` no longer exists.

### **Toolchain Notes (post upstream-sync, v1.12.22 → develop)**

- **Formatter:** oxfmt (config `.oxfmtrc.jsonc`, byte-identical to the old Prettier settings). Do NOT run `prettier`.
- **matrix-js-sdk:** pinned to **`41.8.0`** (npm release) in `apps/web/package.json`. Upstream `develop` floats to `github:matrix-org/matrix-js-sdk#develop`; the fork keeps the validated tagged release. For an upstream PR the rebase onto `develop` naturally restores the float (fork commits never touch the pin).
- **Tests:** both runners are live — Jest (`apps/web` `test:unit` target, snapshots under `test/unit-tests/`) and Vitest (colocated `src/**/*.test.ts`). The fork's ~60 Jest tests stay green; upstream's Vitest migration is additive.
