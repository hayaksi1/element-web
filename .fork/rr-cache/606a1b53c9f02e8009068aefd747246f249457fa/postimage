...
import { act, fireEvent, render, screen, waitFor, waitForElementToBeRemoved } from "test-utils-rtl";
import {
    type HierarchyRoom,
    JoinRule,
    MatrixError,
...
import SpaceHierarchy, { showRoom, toLocalRoom, joinRoom } from "./SpaceHierarchy";
...
describe("SpaceHierarchy", () => {
    describe("showRoom", () => {
        ...
        beforeEach(() => {
            ...
            vi.spyOn(client, "isGuest").mockReturnValue(false);

            vi.spyOn(hierarchy.roomMap, "get").mockReturnValue({
                children_state: [],
                room_id: "room-id2",
    ...
    describe("joinRoom", () => {
        ...
        it("should handle MatrixError exceptions when joining room", async () => {
            ...
            vi.mocked(client.joinRoom).mockRejectedValue(matrixError);
        ...
        it("should handle non-MatrixError exceptions when joining room", async () => {
            ...
            vi.mocked(client.joinRoom).mockRejectedValue(customError);
    ...
    describe("toLocalRoom", () => {
        ...
        vi.spyOn(client, "getRoomUpgradeHistory").mockReturnValue([roomV1, roomV2, roomV3]);
        ...
        describe("If the feature_dynamic_room_predecessors is not enabled", () => {
            ...
            it("Passes through the dynamic predecessor setting", async () => {
                vi.mocked(client.getRoomUpgradeHistory).mockClear();
                const hierarchy = { roomMap: new Map([]) } as RoomHierarchy;
    ...
    describe("<SpaceHierarchy />", () => {
        ...
        vi.mocked(client.getRooms).mockReturnValue([root]);
        ...
        [room1, room2, space1, room3, space2].forEach((r) =>
            vi.mocked(r.getMyMembership).mockReturnValue(KnownMembership.Leave),
        );
        ...
        it("should join subspace when joining nested room", async () => {
            vi.mocked(client.joinRoom).mockResolvedValue({} as Room);
        ...
        it("should offer to join again once the room is left", async () => {
            vi.mocked(client.getRooms).mockReturnValue([root, room1]);
            vi.mocked(room1.getMyMembership).mockReturnValue(KnownMembership.Join);
            ...
            try {
                ...
                vi.mocked(room1.getMyMembership).mockReturnValue(KnownMembership.Leave);
                act(() => {
