...
describe("PlainTextExport", () => {
    ...
    it("should export thread replies alongside the main timeline", async () => {
        ...
        stubRoom.getLiveTimeline = vi.fn().mockReturnValue({ getEvents: () => [message, threadRoot] });
        stubRoom.getThreads = vi.fn().mockReturnValue([
            {
    ...
    it.each([
        ...
        vi.spyOn(SettingsStore, "getValue").mockImplementation((settingName: string): any =>
