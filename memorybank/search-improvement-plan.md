...
1. **In-place, not a list.** ⌘F swaps the chat header for an inline search field; the **live timeline stays**. You
   step through matches with **up/down chevrons**, each click re-scrolls the *same* conversation to the next/previous
...
4. **Telegram's weaknesses we already beat or avoid:** whole-word/prefix only (no substring); **broken for CJK** for
   ...
   (#32038) already makes Seshat CJK-capable — *ahead of Telegram*.
...
### Phase 1 — Fix the "⌘F does nothing" perception *(Small, ~1-2 days, LOW risk)* — RECOMMENDED FIRST
...
### Phase 2 — In-timeline match navigation + live highlight + "k of N" *(Large, ~2-4 wks, HIGH risk)*
...
- Risk: search currently *replaces* the timeline (`timelineRenderingType=Search`); driving the live timeline to
...
### Phase 4 — Typed, searchable shared-media tabs *(Large, ~2-4 wks, MEDIUM-HIGH risk)*
...
### Phase 5 — Reach, ranking option, robustness *(Very Large / exploratory, HIGH risk)*
