...
The element-hq PR targets **`develop`**. The fork's custom work must be expressed as clean commits _on top of_ develop —
**not** the macOS-private bits the project won't accept. Recommended path:

1. **Decide scope.** element-hq is unlikely to take the whole private fork. Likely PR-able, _separable_ pieces:
...
3. **Per-PR hygiene:** conform to upstream conventions (oxfmt, vitest colocated tests for _new_ files, MVVM-v2, named
