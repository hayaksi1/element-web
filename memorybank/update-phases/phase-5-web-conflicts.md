...
> **Risk: LOW. Goal:** resolve the 2 web-side conflicts and _verify_ (not re-merge) the ~13 web auto-merges so every
...
merge (new upstream file). Behavior identical — only the import _source_ changes.
...
- **`apps/web/src/indexing/EventIndex.test.ts`** (content conflict at the new path): adopt upstream's relocated vitest
  ...
  survive.
    - _Alternative if conversion is risky:_ keep the fork's jest test at the **old** path
...
| `shared-components/.../en_EN.json` | fork `room                                                                                                                                                                                                                                                                                                                                                                                             | search`block (match_position/next/previous) present; upstream`release_announcement`+`room_list | a11y                                                                                                | drag\_\*`added;`toggle_unread` reworded. |
...
| `Notifier-test.ts`                 | **keep the +110 #31996 macOS wake-throttle suite** (5 tests, jest._ — do NOT convert to vi._); upstream import rename + `mocked()` wrap applied. Still runs under jest.                                                                                                                                                                                                                                |
