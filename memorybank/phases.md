...
| 3.7 | #32114          | Crash on close (Ventura/M2).                                                         | ❌ **document-only (session 14)**: Electron already bumped to 42.3.3 (14 majors past the crashing 1.11.58); the residual is a native NSMenu→V8 teardown use-after-free (`objc_msgSend` KERN_INVALID_ADDRESS) with no in-repo JS lever. Catalogue's "tray.destroy on quit" is a **no-op on darwin** (no tray on macOS); the ⌘Q/menu/tray quit paths are already clean. Track upstream Electron. |
| 3.8 | #32222 / #32223 | White-screen-on-return; single-instance focus after boot.                            | ✅ **#32222 mitigation done (session 14)**: the white screen itself is an upstream Chromium/GPU renderer crash (corrupted GPUCache), but there was NO `render-process-gone`/`unresponsive` handler in the main process so a dead renderer stayed a permanent blank window. New `renderer-recovery.ts` auto-reloads on crash-class reasons (quit-guarded, 3/60s cap → dialog) + `isCrashed()`-gated reload-before-show on activate/second-instance. **Mitigation, not a root-cause fix.** #32223 (Linux GNOME autostart 2nd instance) stays upstream. **Session 15 (deferred review-fix hardening):** the activate/second-instance relaunch reloads now route through a new capped `RendererRecovery.recoverIfCrashed()` (instead of an inline uncapped reload) so a user relaunch can't re-arm a crash loop the recovery already gave up on; +boundary tests (`abnormal-exit`/`memory-eviction`→ignore, `unresponsive`-at-cap→dialog, cap-fed-through-recoverIfCrashed). renderer-recovery 21→**32 tests**. |
...
| 4.2 | #32341 / #32258 / #32266 / #32011 / #32356 / #32343 | Query/index correctness bugs — triage as a batch against `seshat.ts` + matrix-seshat. | ✅ **mostly done** — #32266/#32011 (session 12, `reconcileMissedRooms` per PR #33955, see below); **#32341 / #32258 / #32356 done (session 13, `apps/web/src/Searching.ts`)**; **#32343 document-only/upstream**. **Session 13 (read-path only, 27 Jest tests):** (a) **#32341** URL/colon search — `combinedSearch()` now `Promise.allSettled` (degrades to the surviving leg, throws only if both fail) so an All-Rooms search no longer hard-fails when Seshat rejects the tantivy `field:` operator; plus `hardenSeshatSearchTerm()` phrase-wraps a colon-bearing term for the **Seshat leg only** (closed-phrase-aware so an unbalanced leading quote is escaped, not passed through). (b) **#32258** upgraded-room history — `getRoomSearchChain()` walks `findPredecessor()` (cycle-guarded, depth-cap 20); single-room search partitions the chain by **per-room** encryption: encrypted rooms via Seshat (`chainSearchProcess` k-way recency merge over per-room queries), known non-encrypted predecessors via the homeserver (`filter.rooms`), merged — covers the mixed-encryption upgrade. (c) **#32356** edited messages — a matched `m.replace` result is **re-keyed to its target (original) event id** (so the SDK event mapper resolves the renderable original instead of reusing the live `m.replace`, which is dropped by `haveRendererForEvent`) and its `m.new_content` promoted for the not-loaded case; results de-duped (edit re-keyed to original alongside the original) and an empty `m.new_content` is left untouched. **#32343 (non-stopwords): UPSTREAM** — pure native tantivy tokenizer (`SimpleTokenizer`+`LowerCaser`+`RemoveLongFilter`), same family as #32038; no in-repo TS query bug. Documented (code comment), not fixed in-repo: the degraded combinedSearch leg is sticky for the session (sound because each leg ≤ SEARCH_LIMIT, so the degraded page never overflows cachedEvents). |
| 4.3 | #32038 / #32119 / #32112 / #32130                   | i18n tokenization, CPU spikes, packaging, Skylake illegal-instruction.                | ⬆️ mostly upstream; ✅ **#33954 adopted (session 11), BUILD-VERIFIED (session 16)**: arm64 `--cfg aes_armv8` RUSTFLAGS in `hak/matrix-seshat/build.ts`. **Session 16 on M4 Pro:** flag compiles clean (Rust 1.95, exit 0, no warnings — aes 0.8.4 self-declares the cfg); confirmed it flips the Rust `aes` crate to the ARMv8 hardware backend (artifact ARMv8 AES instr 225→426; standalone aes crate 0→42 via `otool`); rebuilt the real seshat `index.node` (was shipping the **software**-AES build — fingerprint `rustflags:[]`) and validated a full encrypted encrypt→commit→reopen→decrypt→search round-trip + wrong-passphrase rejection. New artifact in `.hak/hakModules/`. **Remaining:** repackage the `.app` to ship it + measure live CPU drop under indexing (manual QA — `manual-qa-checklist.md`). ✅ **#32038 N-gram tokenizer done (session 14, adapted from PR #33048)**: new `tokenizerMode` device+CONFIG setting (language\|ngram) threaded web→IPC→`seshat.ts` into the `Seshat` constructor config; the locally-built **matrix-seshat 4.3.0** binding already exposes `tokenizerMode`/`ngram` (offline-satisfied — earlier "blocked on 4.2.0 bump" was stale). New pure DI `seshat-config.ts`/`seshat-index.ts`; the desktop persists the mode in `Store` and **rebuilds the index on change** (rebuildable cache, no history lost). Per-user dropdown + confirm-reindex dialog DEFERRED (config.json-reachable for now). Also the real fix for **#32343** (non-stopwords). |
...
| 5.2 | #31996 | macOS Sequoia: notification sounds stack after wake.                          | ✅ **done (session 14)**: root cause = the renderer Web-Audio path (`Notifier.playAudioNotification`→`BackgroundAudio`) had no throttle, so a wake-from-sleep sync backlog fired many identical sounds at once → superimposed "stacked" sound. Fix: a **per-sound throttle** (`Map<soundKey,lastMs>`, 1000ms, keyed on resolved sound url, AFTER the silence gate) coalesces identical backlogged sounds to one play while distinct sounds still play. **Does NOT** cure the variant where macOS Sequoia ignores `silent:true` and plays its OWN coalesced banner sound (OS-only, documented). |
...
- **Phase 4.2 query-correctness bugs: DONE (session 13)** — #32341 (All-Rooms URL/colon resilience + Seshat term
  hardening), #32258 (predecessor-chain traversal, incl. mixed-encryption), #32356 (edited-message rendering via
  original-id re-key) all landed in `apps/web/src/Searching.ts` (27 Jest tests, 8-dimension+verify adversarial review →
  7 findings, 5 fixed / 2 documented). **#32343 (non-stopwords) = UPSTREAM** (native tantivy tokenizer, same as #32038).
- **Still planned / untouched:** **#33954 native arm64 QA** (only unverified earlier change); Phase 4.3 **#33048** N-gram
  tokenizer for CJK/#32038 (needs matrix-seshat 4.2.0 bump under the offline constraint — also the real fix for #32343);
  Phase 5.1 macOS DND/Focus (needs a vetted native module); Phase 5.2 Sequoia notification-sound stacking; Phase 3.7/3.8
  (Electron-bump / upstream). **Known/documented limitation (session 13):** `combinedSearch` degradation is sticky for
  the search session (a transiently-failed leg isn't retried on later pages) — sound under the current SEARCH_LIMIT cap.
