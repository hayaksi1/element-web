...
- **Refuted** the workflow's second guess (onSearchUpdate fires _during_ stepping): the `RoomSearchView` data engine
...
`onSearchUpdate(false, results, …)` landing _while a match is pinned_ (the real async search resolving at/after the
...
- Codex adversarial review: 1 Medium (focusedMatch/cursor split if the focused match vanishes) → FIXED + tested;
  ...
  accepted, stepping to _different_ matches re-fires because tiles are keyed by event id — deferred).
