...
export default class RightPanelStore extends ReadyWatchingStore {
    ...
    protected onDispatcherAction(payload: ActionPayload): void {
        switch (payload.action) {
            ...
            // NB: Action.FocusMessageSearch used to open the RoomSummary card with the search box focused. As of
            // search Phase 6 the in-room search lives in a Telegram-style top-of-chat bar owned by RoomView, so the
            // right panel no longer reacts to it.
        }
    }
