...
describe("<LoggedInView />", () => {
    ...
    it("shows the one-time in-room search nudge on Ctrl+F when the shortcut is disabled (web)", async () => {
        ...
        getComponent();
        // Dispatch on document.body directly: the nudge is deliberately scoped to the "nothing focused" path, and
        // userEvent.keyboard leaves a gap between the Control and F keydowns in which an unrelated dialog rendered by
        // this suite takes focus, so the F keydown would no longer target the body.
    ...
    it("does not show the in-room search nudge when the shortcut is enabled", async () => {
        ...
        getComponent();
        // Same body-targeted dispatch as the positive case above, so this asserts the setting actually suppresses the
        // nudge rather than passing vacuously because the keydown never reached the handler.
        fireEvent.keyDown(document.body, { key: "f", code: "KeyF", ctrlKey: true });
    ...
    describe("chat background", () => {
        ...
        const getChatBody = (container: HTMLElement): HTMLElement => container.querySelector(".mx_MatrixChat")!;
