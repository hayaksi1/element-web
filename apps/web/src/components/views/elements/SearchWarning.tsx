 ...
 * ({@link EventIndex.checkpointedRooms}, which covers both the checkpoint being crawled right now
 * and those still queued behind it), or if the index holds no events for it at all
...
function useIsIndexIncomplete(index: EventIndex | null, scope?: SearchScope, roomId?: string): boolean {
    const readCheckpoints = useCallback((): { relevant: boolean; anyOutstanding: boolean } => {
        ...
        const checkpointed = index.checkpointedRooms();
        ...
        return {
            relevant: roomScoped ? checkpointed.has(roomId) : checkpointed.size > 0,
            anyOutstanding: checkpointed.size > 0,
        };
    }, [index, scope, roomId]);
