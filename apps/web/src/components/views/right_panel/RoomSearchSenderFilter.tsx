 ...
 * search input and the jump-to-date calendar (search Phase 3 slice 2).
 *
 ...
 * post-filter). The selected senders live in the {@link SearchSessionStore} (passed in via `senders`) so they
 * survive RoomView remounts during cross-room match stepping. Renders nothing when the room has no other members
 * (nobody to filter by).
 */
        ...
        <Menu
            ...
            trigger={
                <IconButton
                    ...
                    // accessible name when a filter is set (review follow-up).
                    aria-label={
                        senders.length > 0
