...
import { act, fireEvent, render, type RenderResult, waitForElementToBeRemoved, waitFor } from "test-utils-rtl";
import {
    MatrixEvent,
    Relations,
...
import MPollBody, { allVotes, findTopAnswer, isPollEnded } from "./MPollBody";
...
const mockClient = getMockClientWithEventEmitter({
    ...
    sendEvent: vi.fn().mockReturnValue(Promise.resolve({ event_id: "fake_send_id" })),
    getRoom: vi.fn(),
...
describe("MPollBody", () => {
    ...
    it("shows scores if the poll is undisclosed but ended", async () => {
        ...
        expect(hasWinnerIcon(renderResult, "pizza")).toBe(true);
    ...
    it("counts votes as normal if the poll is ended", async () => {
        ...
        expect(hasWinnerIcon(renderResult, "poutine")).toBe(true);
        expect(endedVotesCount(renderResult, "poutine")).toBe("1 vote");
        expect(endedVotesCount(renderResult, "italian")).toBe("0 votes");
        expect(hasWinnerIcon(renderResult, "wings")).toBe(true);
        expect(endedVotesCount(renderResult, "wings")).toBe("1 vote");
    ...
    it("renders a warning message when poll has undecryptable relations", async () => {
        ...
        vi.spyOn(votes[1], "isDecryptionFailure").mockReturnValue(true);
...
function votesCount({ getByTestId }: RenderResult, value: string): string {
    return getByTestId(`pollOption-${value}`).querySelector(".mx_PollOption_optionVoteCount")!.textContent!;
}

function hasWinnerIcon({ getByTestId }: RenderResult, value: string): boolean {
    return !!getByTestId(`pollOption-${value}`).querySelector(".mx_PollOption_winnerIcon");
}
