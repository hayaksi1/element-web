...
/**
 * MVVM-v2 view model backing the search `from:`/sender filter picker (search Phase 3 slice 2).
 *
 * Owns the candidate sender catalogue — the room's joined members, minus the current user, sorted by display
 * name — so the View stays dumb. The selected senders are NOT held here: they live in the
 * {@link SearchSessionStore} (threaded down as a prop) so they survive RoomView remounts during cross-room
 * stepping; the View toggles them through an injected callback.
 */
