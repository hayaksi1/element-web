...
describe("EventIndex", () => {
    ...
    describe("onTimelineReset re-seeding (#33957)", () => {
        ...
        async function setUpIndexer(
            ...
            const mockClient = getMockClientWithEventEmitter({
                ...
                getCrypto: () =>
                    ({
                        isEncryptionEnabledInRoom: jest.fn().mockResolvedValue(encryptionEnabled),
                    }) as any,
    ...
    describe("reconcileMissedRooms (#32266, #32011)", () => {
        interface ReconcileSetup {
            indexer: EventIndex;
            addCrawlerCheckpoint: jest.Mock;
            addEventToIndex: jest.Mock;
            isRoomIndexed: jest.Mock;
            getCrypto: jest.Mock;
        ...
        async function setUpReconcile(
            ...
            const addCrawlerCheckpoint = jest.fn();
            const addEventToIndex = jest.fn();
            const isRoomIndexed = jest.fn((roomId: string) => Promise.resolve(resolve(roomIndexed, roomId)));

            // Recreated per sync so we can await several syncs (e.g. crypto-not-ready retry).
            let commitResolve: (() => void) | undefined;
            const mockIndexingManager = {
                loadCheckpoints: jest.fn().mockResolvedValue(loadCheckpoints),
                isEventIndexEmpty: jest.fn().mockResolvedValue(false),
                isRoomIndexed,
                addCrawlerCheckpoint,
                removeCrawlerCheckpoint: jest.fn(),
                addEventToIndex,
                commitLiveEvents: jest.fn().mockImplementation(async () => commitResolve?.()),
            } as any as Mocked<BaseEventIndexManager>;
            mockPlatformPeg({ getEventIndexingManager: () => mockIndexingManager });

            const isEncryptionEnabledInRoom = jest.fn((roomId: string) =>
                Promise.resolve(resolve(encryptionEnabled, roomId)),
            );
            const getCrypto = jest.fn().mockReturnValue({ isEncryptionEnabledInRoom });
            const mockClient = getMockClientWithEventEmitter({
                getEventMapper: () => (obj: Partial<IEvent>) => new MatrixEvent(obj),
                createMessagesRequest: jest.fn(),
                ...
                isRoomEncrypted: jest.fn((roomId: string) => resolve(roomEncrypted, roomId)),
            });
    ...
    describe("crawler error handling and fully-crawled marker (#32119)", () => {
        function setUpCrawler({ checkpoints, rooms }: { checkpoints: ICrawlerCheckpoint[]; rooms: Room[] }): {
            mockClient: Mocked<MatrixClient>;
            addHistoricEvents: jest.Mock;
            removeCrawlerCheckpoint: jest.Mock;
        } {
            const addHistoricEvents = jest.fn().mockResolvedValue(false);
            const removeCrawlerCheckpoint = jest.fn().mockResolvedValue(undefined);
            const mockIndexingManager = {
                loadCheckpoints: jest.fn().mockResolvedValue(checkpoints),
                isEventIndexEmpty: jest.fn().mockResolvedValue(false),
                addHistoricEvents,
    ...
    describe("duplicate checkpoint guard (hasQueuedCheckpoint)", () => {
        it("does not queue an exact-duplicate checkpoint twice", async () => {
            ...
            const mockClient = getMockClientWithEventEmitter({
                getEventMapper: () => (obj: Partial<IEvent>) => new MatrixEvent(obj),
                createMessagesRequest: jest.fn(),
                getCrypto: () => ({ isEncryptionEnabledInRoom: jest.fn().mockResolvedValue(true) }) as any,
                ...mockClientMethodsRooms([room]),
                isRoomEncrypted: jest.fn().mockReturnValue(true),
            });
    ...
    describe("getIndexingStatus (#33956)", () => {
        it("counts indexing/indexed joined encrypted rooms and ignores invites and unencrypted rooms", async () => {
            ...
            getMockClientWithEventEmitter({
                ...
                createMessagesRequest: jest.fn(),
                ...mockClientMethodsRooms(rooms),
                isRoomEncrypted: jest.fn((roomId: string) => roomId !== "!plain:id"),
            });
    ...
    describe("onRoomStateEvent newly-encrypted seeding", () => {
        async function setUp(
            ...
            const mockIndexingManager = {
                loadCheckpoints: jest.fn().mockResolvedValue([]),
                isEventIndexEmpty: jest.fn().mockResolvedValue(false),
                isRoomIndexed: jest.fn().mockResolvedValue(false),
                addCrawlerCheckpoint,
                removeCrawlerCheckpoint: jest.fn(),
                commitLiveEvents: jest.fn().mockImplementation(async () => commitResolve?.()),
            } as any as Mocked<BaseEventIndexManager>;
            ...
            const mockClient = getMockClientWithEventEmitter({
                getEventMapper: () => (obj: Partial<IEvent>) => new MatrixEvent(obj),
                createMessagesRequest: jest.fn(),
                getCrypto: () => ({ isEncryptionEnabledInRoom: jest.fn().mockResolvedValue(encryptionEnabled) }) as any,
                ...
                ...mockClientMethodsRooms([]),
                getRoom: jest.fn((id: string) => (id === "!new:id" ? newRoom : null)),
                isRoomEncrypted: jest.fn().mockReturnValue(true),
            });
