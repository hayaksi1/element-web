...
import { type ResolveDefaults, type DesktopConfigJson, type JsonDocument, type JsonObject } from "shared-types";
...
export function loadMergedLocalConfig(opts: ConfigPathOptions): JsonObject {
    ...
    for (const candidate of [...candidates].reverse()) {
        let layer: JsonDocument;
        try {
            layer = loadJsonFile<JsonDocument>(candidate);
        } catch (e) {
...
// Loads the config from asar, and applies a local config.json (per-user `userData`, with any
// machine-wide/MDM layer merged underneath) atop if one exists. Idempotent, returns the same
...
export function loadConfig(localConfigPath: string | undefined): Promise<ConfigOptions> {
    ...
    async function actuallyLoadConfig(): Promise<ConfigOptions> {
        ...
        try {
            // Load local config (per-user `userData` config, with any machine-wide/MDM layer merged
            // underneath) and use it to override values from the one baked with the build.
            const localConfig = loadMergedLocalConfig({
                platform: process.platform,
                userDataPath: app.getPath("userData"),
                productName: app.getName(),
                env: process.env,
                explicitLocation: localConfigPath,
            });
